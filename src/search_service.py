...
class SearchService:
    ...
    @staticmethod
    def _normalize_relevance_text(value: Optional[str]) -> str:
        return re.sub(r"\s+", " ", str(value or "")).strip().lower()

    @classmethod
    def _score_news_result_relevance(
        cls,
        item: SearchResult,
        *,
        stock_code: str,
        ...
        haystack = cls._normalize_relevance_text(
            " ".join(filter(None, [item.title, item.snippet, item.source, item.url]))
        )
        if not haystack:
            return 0.0

        score = 0.0
        normalized_code = (stock_code or "").strip().lower()
        normalized_name = cls._normalize_relevance_text(stock_name)

        if normalized_code and normalized_code in haystack:
            score += 2.5
        ...
        score += min(2.0, sum(1.0 for token in name_tokens if token in haystack))

        for keyword in focus_keywords or []:
            normalized_keyword = cls._normalize_relevance_text(keyword)
            if normalized_keyword and normalized_keyword in haystack:
                score += 0.8

        if normalized_name and normalized_name not in haystack and normalized_code and normalized_code not in haystack:
            score -= 1.0
    ...
    def _filter_relevant_news_response(
        ...
        *,
        ...
        scored_items: List[Tuple[float, SearchResult]] = []
        for item in response.results:
            score = cls._score_news_result_relevance(
                item,
        ...
        filtered = [item for _, item in scored_items[:max_results]]
        return (
            SearchResponse(
                query=response.query,
                results=filtered,
        ...
        try:
            ...
            for provider in self._providers:
                ...
                if filtered_response.success and filtered_response.results:
                    ...
                    relevant_response, relevant_count = self._filter_relevant_news_response(
