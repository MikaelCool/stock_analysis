...
class LLMToolAdapter:
    ...
    def _call_litellm_model(
        ...
        *,
        ...
        if model.startswith("openai/") and not parsed.content and not parsed.tool_calls:
            logger.warning(
                "Agent LLM %s returned empty content/tool_calls; retrying via OpenAI responses stream fallback",
                model,
            )
            return self._call_openai_responses_stream_fallback(
                messages=openai_messages,
                model=model,
                timeout=timeout,
            )
    ...
    def _resolve_openai_transport_params(self, model: str) -> Dict[str, Any]:
        """Resolve api_key/api_base for an OpenAI-compatible model."""
        for entry in self._config.llm_model_list or []:
            if (entry.get("model_name") or "").strip() != model:
                continue
        ...
        keys = get_api_keys_for_model(model, self._config)
        extra_params = extra_litellm_params(model, self._config)
        return {
    ...
    @staticmethod
    def _messages_to_responses_prompt(messages: List[Dict[str, Any]]) -> str:
        """Flatten chat history into a text prompt for responses-stream fallback."""
        lines: List[str] = []
        for msg in messages:
            ...
            if role == "assistant" and msg.get("tool_calls"):
                ...
                lines.append(f"assistant_tool_calls: {tool_names}")
            if content_text:
                lines.append(f"{role}: {content_text}")

        return "\n\n".join(lines).strip()

    def _call_openai_responses_stream_fallback(
        self,
        *,
        messages: List[Dict[str, Any]],
        model: str,
        ...
        """Fallback for OpenAI-compatible gateways that only emit text on responses streaming."""
        params = self._resolve_openai_transport_params(model)
        api_key = params.get("api_key")
        api_base = params.get("api_base")
        ...
        prompt = self._messages_to_responses_prompt(messages)
        ...
        if not text:
            raise ValueError("responses stream returned empty response")
