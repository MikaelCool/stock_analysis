...
"""
    ...
    def _resolve_openai_transport_params(self, model: str, config: Config) -> Dict[str, Any]:
    ...
    def _call_openai_responses_stream_fallback(
        ...
        *,
        ...
        params = self._resolve_openai_transport_params(model, config)
        api_key = params.get("api_key")
        api_base = params.get("api_base")
        ...
        for model in models_to_try:
            try:
                ...
                if model.startswith("openai/"):
                    logger.warning(
                        "[LiteLLM] %s returned empty content; retrying via OpenAI responses stream fallback",
                        model,
                    )
                    response_text, usage = self._call_openai_responses_stream_fallback(
                        model=model,
                        prompt=prompt,
