...
class TestAnalyzerGenerateText:
    ...
    def test_call_litellm_empty_openai_response_uses_responses_stream_fallback(self):
        analyzer = self._make_analyzer()
        analyzer._config_override = SimpleNamespace(
        ...
        with patch.object(analyzer, "_dispatch_litellm_completion", return_value=empty_response), \
             patch.object(
                 analyzer,
                 "_call_openai_responses_stream_fallback",
                 return_value=("fallback text", {"prompt_tokens": 1, "completion_tokens": 2, "total_tokens": 3}),
             ) as mock_fallback:
            text, model, usage = analyzer._call_litellm(
                "prompt",
                {"max_tokens": 128, "temperature": 0.2},
    ...
    def test_responses_stream_fallback_aggregates_text_and_usage(self):
        analyzer = self._make_analyzer()
        cfg = SimpleNamespace(
            llm_model_list=[
                {
        ...
        with patch("openai.OpenAI", return_value=stream_client):
            text, usage = analyzer._call_openai_responses_stream_fallback(
        ...
        assert text == "hello"
        assert usage == {"prompt_tokens": 7, "completion_tokens": 8, "total_tokens": 15}
