...
class TestAgentConstructionChain(unittest.TestCase):
    ...
    @patch("src.agent.llm_adapter.Router")
    def test_llm_adapter_empty_openai_response_uses_responses_stream_fallback(self, _mock_router):
        """Agent adapter should recover when OpenAI-compatible non-stream replies are empty."""
        mock_cfg = MagicMock()
        mock_cfg.agent_litellm_model = "openai/gpt-5.4"
        mock_cfg.litellm_model = None
        ...
        with patch("src.agent.llm_adapter.get_effective_agent_primary_model", return_value="openai/gpt-5.4"), \
             patch.object(adapter, "_parse_litellm_response", return_value=SimpleNamespace(content=None, tool_calls=[])), \
             patch.object(adapter, "_call_openai_responses_stream_fallback") as mock_fallback:
            mock_fallback.return_value = SimpleNamespace(content="fallback reply", tool_calls=[], usage={})
            with patch.object(adapter._router, "completion", return_value=empty_response):
                result = adapter.call_completion(messages=[{"role": "user", "content": "hi"}], tools=[])
    ...
    @patch("src.agent.llm_adapter.Router")
    def test_agent_responses_stream_fallback_aggregates_text(self, _mock_router):
        """Responses stream fallback should assemble text and usage into LLMResponse."""
        mock_cfg = MagicMock()
        mock_cfg.agent_litellm_model = "openai/gpt-5.4"
        mock_cfg.litellm_model = None
        mock_cfg.litellm_fallback_models = []
        ...
        completed_usage = SimpleNamespace(input_tokens=7, output_tokens=8, total_tokens=15)
        fake_events = iter([
            SimpleNamespace(type="response.created"),
            SimpleNamespace(type="response.output_text.delta", delta="hel"),
            SimpleNamespace(type="response.output_text.delta", delta="lo"),
            SimpleNamespace(type="response.completed", response=SimpleNamespace(usage=completed_usage)),
        ])
        stream_client = MagicMock()
        stream_client.responses.create.return_value = fake_events

        with patch("src.agent.llm_adapter.OpenAI", return_value=stream_client):
            result = adapter._call_openai_responses_stream_fallback(
        ...
        self.assertEqual(result.usage, {"prompt_tokens": 7, "completion_tokens": 8, "total_tokens": 15})
        self.assertEqual(result.model, "openai/gpt-5.4")
        stream_client.responses.create.assert_called_once()
