...
class DataFetcherManager:
    ...
    def _cache_root(self) -> Path:
        root = Path(__file__).resolve().parent.parent / "data" / "cache" / "fetcher_manager"
        root.mkdir(parents=True, exist_ok=True)
        return root

    @staticmethod
    def _cache_day_key() -> str:
        return datetime.now().strftime("%Y%m%d")

    @staticmethod
    def _should_use_local_eod_cache(market: str) -> bool:
        now = datetime.now()
        if market == "cn":
            return now.weekday() >= 5 or (now.hour > 15 or (now.hour == 15 and now.minute >= 5))
        return True

    def _cache_file(self, namespace: str, key: str) -> Path:
        namespace_dir = self._cache_root() / namespace
    ...
    def _read_snapshot_cache(self, namespace: str, key: str, *, market: str) -> Optional[Dict[str, Any]]:
        if not self._should_use_local_eod_cache(market):
            return None
        ...
        try:
            ...
            logger.debug("[本地缓存] 读取失败 %s: %s", cache_file, exc)
            return None

        if str(payload.get("cache_date") or "") != self._cache_day_key():
            return None
        return payload
    ...
    def _write_snapshot_cache(
        ...
        *,
        ...
        cache_file = self._cache_file(namespace, key)
        data = {
            "cache_date": self._cache_day_key(),
            "market": market,
            **payload,
        }
        try:
            ...
            logger.debug("[本地缓存] 写入失败 %s: %s", cache_file, exc)
    ...
    def get_daily_data(
        ...
        cached_daily = self._read_snapshot_cache("daily", cache_key, market=market)
        if cached_daily and isinstance(cached_daily.get("records"), list):
            try:
                cached_df = pd.DataFrame(cached_daily["records"])
                if not cached_df.empty and "date" in cached_df.columns:
                    cached_df["date"] = pd.to_datetime(cached_df["date"])
                source_name = str(cached_daily.get("source") or "local_cache")
                logger.info(f"[本地缓存] {stock_code} 日线命中: source={source_name}")
                return cached_df, source_name
            except Exception as exc:
                logger.debug(f"[本地缓存] {stock_code} 日线反序列化失败: {exc}")
        ...
        if market == "cn" and tickflow_fetcher is not None:
            try:
                logger.info(f"[数据源尝试] [TickFlowFetcher] 获取 {stock_code} 日线...")
                df = self._call_fetcher_method(
                    tickflow_fetcher,
                    "get_daily_data",
                    stock_code=stock_code,
                ...
                if df is not None and not df.empty:
                    ...
                    elapsed = time.time() - request_start
                    logger.info(
                        f"[数据源完成] {stock_code} 使用 [TickFlowFetcher] 获取成功: "
                        f"rows={len(df)}, elapsed={elapsed:.2f}s"
                    )
                    return df, "TickFlowFetcher"
            except Exception as e:
                error_type, error_reason = summarize_exception(e)
                errors.append(f"[TickFlowFetcher] ({error_type}) {error_reason}")
                logger.warning(
                    f"[数据源失败] [TickFlowFetcher] {stock_code}: error_type={error_type}, reason={error_reason}"
                )
    ...
    def get_realtime_quote(self, stock_code: str, *, log_final_failure: bool = True):
        ...
        if market == "cn":
            ...
            if tickflow_fetcher is not None:
                try:
                    quote = self._call_fetcher_method(tickflow_fetcher, 'get_realtime_quote', raw_stock_code or stock_code)
                    if quote is not None and quote.has_basic_data():
                        ...
                        if not self._quote_needs_supplement(primary_quote):
                            self._write_snapshot_cache(
                                ...
                                cache_key,
                                market=market,
                                payload={"quote": primary_quote.to_dict()},
                            )
                            return primary_quote
                except Exception as e:
                    error_msg = f"[tickflow] 失败: {str(e)}"
            ...
            try:
                ...
                if data:
                    self._write_snapshot_cache(
                        ...
                        cache_key,
                        market=region,
                        payload={"data": data, "source": "TickFlowFetcher"},
                    )
            ...
            try:
                ...
                if data:
                    self._write_snapshot_cache(
                        ...
                        cache_key,
                        market=region,
                        payload={"data": data, "source": fetcher.name},
                    )
