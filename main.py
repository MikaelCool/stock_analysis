...
def _configure_utf8_stdio() -> None:
    """Force UTF-8 stdio on Windows so Chinese logs do not become mojibake."""
...
def run_full_analysis(
    ...
    try:
        ...
        # === Auto stock picker scan ===
        try:
            if (
                getattr(config, 'stock_picker_enabled', True)
                and getattr(config, 'stock_picker_schedule_enabled', True)
            ):
                from src.services.stock_picker_service import StockPickerService

                logger.info("开始自动执行收盘后选股扫描...")
                picker_payload = StockPickerService(config=config).run_scheduled_scan()
                logger.info(
                    "自动选股完成: strategy=%s scan_date=%s selected=%s",
                    picker_payload.get('strategy_name'),
                    picker_payload.get('scan_date'),
                    picker_payload.get('selected_count'),
                )
        except Exception as e:
            logger.warning(f"自动选股扫描失败（已忽略）: {e}")
...
def _build_stock_picker_schedule_checker():
    ...
    def _checker() -> None:
        ...
        if not getattr(runtime_config, "stock_picker_enabled", True):
            return
        if not getattr(runtime_config, "stock_picker_schedule_enabled", True):
            return

        schedule_time = (getattr(runtime_config, "stock_picker_schedule_time", "16:00") or "16:00").strip() or "16:00"
        try:
        ...
        if args.schedule or config.schedule_enabled:
            ...
            if getattr(config, 'stock_picker_enabled', True) and getattr(config, 'stock_picker_schedule_enabled', True):
                background_tasks.append({
                    "task": _build_stock_picker_schedule_checker(),
                    "interval_seconds": 30,
                    "run_immediately": True,
                    "name": "stock_picker_schedule",
                })
